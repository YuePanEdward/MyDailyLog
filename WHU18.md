...
## 编译
编译的时候先 预处理 include 各种头文件过来，这时候如果在指定路径下找不到头文件(.h/.hpp) 那就会凉凉，报找不到xxx.h文件的错

然后预处理完要汇编，汇编完的.obj文件得和其他lib.文件链接(Link)在一起,这时候要是找不到lib文件，就会报Link的错，这是候就要好好看看库目录对不对了。
